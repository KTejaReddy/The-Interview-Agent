...
def test_models_reject_json_mode_hook() -> None:
    """models flagged json_mode=False so the provider skips
    response_format for it."""
    from services.llm_service import _model_supports_json

    assert _model_supports_json("qwen/qwen3.6-27b") is False
    assert _model_supports_json("openai/gpt-oss-20b") is False
    assert _model_supports_json("openai/gpt-oss-120b") is False
    assert _model_supports_json("allam-2-7b") is False
    assert _model_supports_json("groq/compound") is False
...
def test_router_skips_unhealthy_model_without_blocking_others() -> None:
    ...
    assert candidates[0] == "openai/gpt-oss-120b"  # next healthy in the pool
